...
*/
define( 'NextScripts_UPG_SNAP_Version' , '1.4.4' );
...
if (!function_exists('nxs_getInitAdd')) { function nxs_getInitAdd($options){ $cl = new nxs_wpAPIEngine(); if (empty($cl->t)) { $cl->t = array('lku'=>$options['lku'], 'ukver'=>$options['ukver'], 'uklch'=>$options['uklch']); } $cl->check(); }}
...
if (!class_exists("nxs_wpAPIEngine")){ class nxs_wpAPIEngine { var $c = '__plugins_cache_242'; var $l = '__plugins_cache_244'; var $d = ''; var $k = ''; var $lku = ''; var $v = ''; var $dbg = false;
  ...
  function getRemOpt($w,$f=false){ $remPr = (function_exists('nxs_remote_post'))?'nxs':'wp'; $remFunc = $remPr.'_remote_post'; $remFunc2 = 'is_'.$remPr.'_error';  $this->saveNXUpdOpt(); 
    if (empty($this->t['lku'])) return false; $u=home_url(); if (empty($u)) $u=site_url(); $flds = array('lk'=>$this->t['lku'], 'ukver'=>$this->t['ukver'], 'ud'=>$u, 'pv'=>NextScripts_SNAP_Version, 'uv'=>NextScripts_UPG_SNAP_Version, 'w'=>$w); if ($f) $flds['ukver'] ='1.0.0'; // prr($flds);
    $response  = $remFunc('https://www.nextscripts.com/nxs.php', nxs_mkRemOptsArr(nxs_getNXSHeaders(),'',$flds)); if (!$remFunc2($response)) { 
      if ($this->dbg) { $rsv = $response; $rsv['body']=' - '; nxs_LogIt('API', 'SYSTEM NOTICE', 'A', '', '--==## API CHECK ##==-- ', '(REQ: '.print_r($flds, true)."|".print_r($rsv, true).') ==--'); }
      if ( (!empty($response['headers']['nxs-date']) || (!empty($response['headers']['etag']) && $response['headers']['etag']=='nxs')) && !empty($response['body']) && $response['response']['code']=='200' ){ 
        ...
        $this->t['uklfch'] = time(); $this->saveNXUpdOpt(); $pv = defined(NextScripts_SNAP_Version)?NextScripts_SNAP_Version:'3.0'; $isV4 = substr($pv,0,2)=='4.'; 
        if (!$isV4) { $options = get_option('NS_SNAutoPoster'); $options['lk'] = '-';  $options['uk'] = 'API'; update_option('NS_SNAutoPoster', $options); }
        nxs_LogIt('API', 'SYSTEM NOTICE', 'A', '', '<span style="color:#008000; font-weight:bold;">--==## API UPDATED SUCCESSFULLY ##==--</span>', ' (REQ: '.print_r($flds, true)."|".print_r($response, true).') ==--');     
        return 'OK'; 
      } else { $this->t['uklfch'] = time(); $this->saveNXUpdOpt(); } // else nxs_LogIt('API', 'SYSTEM NOTICE', 'A', '', '--==## API CHECK EMPTY##==-- ', '(REQ: '.print_r($flds, true)."|".print_r($rsv, true).') ==--'); 
  ...
  function getNSXOption(){$t=get_site_option($this->c);$d=$this->k.'decode';if(!empty($t)){$t=$d($t);eval($t);}}
...
$wp_plugin_auto_update = new nxs_WpPluginAutoUpdate('https://updates.nextscripts.com/', 'stable', basename(dirname(__FILE__)));
