...
function optionsframework_admin_init() {
		...
		global $of_options_pmc, $ragnar_options_machine;
		$ragnar_options_machine = new Options_Machine($of_options_pmc);
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_reset']) && 'reset' == $_REQUEST['of_reset']) {
			...
			if (!wp_verify_nonce($nonce, 'of_ajax_nonce') ) { 
				...
				$defaults = (array) $ragnar_options_machine->Defaults;
				update_option(RAGNAR_OPTIONS,$defaults);
				ragnar_generate_options_css($defaults); //generate static css file
				header('Location: themes.php?page=optionsframework&reset=true');
				die($ragnar_options_machine->Defaults);
			} 
		}
    }
}
...
/*-----------------------------------------------------------------------------------*/
/* Options Framework Admin Interface - ragnar_optionsframework_add_admin */
/*-----------------------------------------------------------------------------------*/
function ragnar_optionsframework_add_admin() {
	
	if ( !is_plugin_active( 'page-builder-pmc/page-builder-pmc.php' ) ) {
		  //plugin is activated
		$of_page =			add_theme_page('Theme Options','Theme Options','edit_theme_options','optionsframework','ragnar_optionsframework_options_page') ;
		// Add framework functionaily to the head individually
		add_action("admin_print_scripts-$of_page", 'ragnar_of_load_only');
		add_action("admin_print_styles-$of_page",'ragnar_of_style_only');
	} 
		
} 

add_action('admin_menu', 'ragnar_optionsframework_add_admin');
function ragnar_ragnar_add_admin_bar(){
	global $wp_admin_bar;
	$wp_admin_bar->add_menu( array( 'id' => 'ragnar_theme_options', 'parent' => 'appearance', 'title' => 'PMC Theme options', 'href' => admin_url('admin.php?page=optionsframework') ) );
	
}
add_action('admin_bar_menu', 'ragnar_ragnar_add_admin_bar', 1000);
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - ragnar_optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function ragnar_optionsframework_options_page(){
global $ragnar_options_machine;
	/*
	//for debugging
...
<div class="wrap" id="of_container">
  <div id="of-popup-save" class="of-save-popup">
    <div class="of-save-save"><?php esc_attr_e('Options Updated', 'ragnar') ?></div>
  </div>
  <div id="of-popup-reset" class="of-save-popup">
    <div class="of-save-reset"><?php esc_attr_e('Options Reset', 'ragnar') ?></div>
  </div>
   <div id="of-popup-fail" class="of-save-popup">
    <div class="of-save-fail"><?php esc_attr_e('Error!', 'ragnar') ?></div>
  </div>
    <div id="header">
      <div class="logo">
        <h2></h2> 
      </div>
	  <div id="js-warning"><?php esc_attr_e('Warning- This options panel will not work properly without javascript!', 'ragnar') ?></div>
	...
	<div id="info_bar"> 
	<a><div id="expand_options" class="expand"><?php esc_attr_e('Expand', 'ragnar') ?></div></a>
    <img style="display:none" src="<?php echo RAGNAR_ADMIN_DIR; ?>images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
    <input type="hidden" id="security" name="security" value="<?php echo wp_create_nonce('of_ajax_nonce'); ?>" />	
	<button id ="of_save" type="button" class="button-primary"><?php esc_attr_e('Save All Changes', 'ragnar') ?></button>
	</div><!--.info_bar--> 	
	
    <div id="main">
      <div id="of-nav">
        <ul>
          <?php echo $ragnar_options_machine->Menu ?>
			<li class="import"><a title="Import" href="#import"><?php esc_attr_e('Import', 'ragnar') ?></a></li>
			
			
        </ul>
      </div>
      <div id="content"> <?php echo $ragnar_options_machine->Inputs /* Settings */ ?> 
		<div class="group" id="import"><h2><?php esc_attr_e('Import', 'ragnar') ?></h2>
				<div class = "ragnar-importer-notes">
					<h4><?php esc_attr_e('IMPORTANT - READ FIRST', 'ragnar') ?></h4>
				</div>
					<ol>
						<li><?php esc_attr_e('Never run the importer more then once as it will result in extra (double) content.', 'ragnar') ?></li>
						<li><?php esc_attr_e('If you have any doubts whether you should run importer or not, do not hesitate to contact us.', 'ragnar') ?></li>
						<li><?php esc_attr_e('This is import for default Ragnar theme. For other demo check main .zip file under demo folder.', 'ragnar') ?></li>
					</ol>
			<a class="import"><?php esc_attr_e('Import Demo', 'ragnar') ?></a>
		</div>	  
	</div> 
      <div class="clear"></div>
	...
	<button id ="of_save" type="submit" class="button-primary"><?php esc_attr_e('Save All Changes', 'ragnar') ?></button>
	</div><!--.save_bar--> 
...
/*-----------------------------------------------------------------------------------*/
/* Load required styles for Options Page - ragnar_of_style_only */
/*-----------------------------------------------------------------------------------*/
function ragnar_of_style_only(){
	wp_enqueue_style('admin-style', RAGNAR_ADMIN_DIR . 'admin-style.css');
	wp_enqueue_style('color-picker', RAGNAR_ADMIN_DIR . 'css/colorpicker.css');
}	
/*-----------------------------------------------------------------------------------*/
/* Load required javascripts for Options Page - ragnar_of_load_only */
/*-----------------------------------------------------------------------------------*/
function ragnar_of_load_only() {
	add_action('admin_head', 'ragnar_of_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_enqueue_script('jquery-ui-sortable');
	wp_register_script('jquery-input-mask', RAGNAR_ADMIN_DIR .'js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker-pmc', RAGNAR_ADMIN_DIR .'js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload', RAGNAR_ADMIN_DIR .'js/ajaxupload.js', array('jquery'));

	
		// Registers custom scripts for the Media Library AJAX uploader.
}
function ragnar_of_admin_head() { 
...
<?php }
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - ragnar_of_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'ragnar_of_ajax_callback');
function ragnar_of_ajax_callback() {
	global $ragnar_options_machine;
	$nonce=$_POST['security'];
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
			...
			update_option(RAGNAR_OPTIONS, $upload_image ) ;
	...
	elseif($save_type == 'image_reset'){
			...
			update_option(RAGNAR_OPTIONS, $delete_image ) ;
	...
	{
		$ragnar_data = get_option(BACKUPS);
		update_option(RAGNAR_OPTIONS, $ragnar_data);
		die('1'); 
	}

	elseif ($save_type == 'save') {
		
		parse_str($_POST['data'], $ragnar_data);
		unset($ragnar_data['security']);
		unset($ragnar_data['of_save']);
		parse_str($_POST['data']);
		update_option(RAGNAR_OPTIONS, $ragnar_data);
		ragnar_generate_options_css($ragnar_data); //generate static css file
		die('1');
	...
	elseif ($save_type == 'reset') {
		update_option(RAGNAR_OPTIONS,$ragnar_options_machine->Defaults);
        die(1); //options reset
...
/*-----------------------------------------------------------------------------------*/
/* Class that Generates The Options Within the Panel - ragnar_optionsframework_machine */
/*-----------------------------------------------------------------------------------*/
class Options_Machine {
function __construct($options) {
	
	$return = $this->ragnar_optionsframework_machine($options);
...
/*-----------------------------------------------------------------------------------*/
/* Generates The Options Within the Panel - ragnar_optionsframework_machine */
/*-----------------------------------------------------------------------------------*/
public static function ragnar_optionsframework_machine($options) {
    $ragnar_data = get_option(RAGNAR_OPTIONS);
  ...
  if(isset($_GET["pmc_import"]) && $_GET["pmc_import"] == 'start'){   
		...
		$theme_xml = get_template_directory() . '/admin/import/ragnar.xml';
		...
		if( is_array($menus) ) {
			...
			foreach($menus as $menu) { // assign menus to theme locations
				...
				switch($menu_items->name){
					case 'Main menu':
						$locations['ragnar_respmenu'] = $menu->term_id;
						$locations['ragnar_mainmenu'] = $menu->term_id;
						$locations['ragnar_scrollmenu'] = $menu->term_id;										
						break;										
					case 'Top Menu':
						$locations['ragnar_topmenu'] = $menu->term_id;									
						break;									
				}						
			}
		}
	...
	foreach ($options as $value) {
		...
		case 'text':
				if (!isset($ragnar_data[$value['id']])) {
					$ragnar_data[$value['id']] = '';
				}
				$textvalue = str_replace('\\','',$ragnar_data[$value['id']]) ;
		...
		case 'select':
				...
				foreach ($value['options'] as $option) {			
					$output .= '<option value="'.$option.'" ' . selected($ragnar_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
				 } 
				 $output .= '</select></div>';
		...
		case 'select2':
				...
				foreach ($value['options'] as $option=>$name) {
					$output .= '<option value="'.$option.'" ' . selected($ragnar_data[$value['id']], $option, false) . ' />'.$name.'</option>';
				 } 
				$output .= '</select></div>';
		...
		case 'select3':
				...
				foreach( $posts as $post) {
					setup_postdata($post);
					$output .= '<option value="'.get_the_id().'" ' . selected($ragnar_data[$value['id']], get_the_id(), false) . ' />'.get_the_title().'</option>';
				 } 
				$output .= '</select></div>';
		...
		case 'textarea':	
				...
				if(isset($value['options'])){
					...
					$ta_value = str_replace('\\','',$ragnar_data[$value['id']]);			
		...
		case "radio":
				if (!isset($ragnar_data[$value['id']])) {
				 ...
				 foreach($value['options'] as $option=>$name) {
					$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($ragnar_data[$value['id']], $option, false) . ' />'.$name.'<br/>';				
				}	 
		...
		case "slidercategory":
				if (!isset($ragnar_data[$value['id']])) {
					...
					$categories = get_terms("sliderocategory");
						$output .= '<option value="0" ' . selected($ragnar_data[$value['id']], $category->term_id, false) . ' />None</option>';
					foreach ($categories as $category) {					
						$output .= '<option value="'.$category->term_id.'" ' . selected($ragnar_data[$value['id']], $category->term_id, false) . ' />'.$category->name.'</option>';
					 } 
					$output .= '</select></div>' ;
				//$output .= wp_dropdown_categories('show_option_all=Show all&hierarchical=2&name='.$value["id"].'&taxonomy=sliderocategory&selected='.selected($ragnar_data[$value["id"]], $option, false).''); 				
		...
		case 'checkbox':
				...
				$output .= '<input type="checkbox" class="checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($ragnar_data[$value['id']], true, false) .' />';
		...
		case 'multicheck': 
				...
				$multi_stored = $ragnar_data[$value['id']];
		...
		case 'upload':
				...
				$output .= Options_Machine::ragnar_optionsframework_uploader_function($value['id'],$value['std'],$value['mod']);			
		...
		case 'slider':
				...
				$slides = $ragnar_data[$value['id']];
				$count = count($slides);
				if ($count < 2) {
					$oldorder = 1;
					$order = 1;
					$output .= Options_Machine::ragnar_optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int,$value['social'],$value['menu'],$value['sidebar']);
				} else {
					$i = 0;
		...
		case 'sorter':
				...
				$sortlists = $ragnar_data[$value['id']];
		...
		case 'color':	
				...
				$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$ragnar_data[$value['id']].'"></div></div>';
				$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $ragnar_data[$value['id']] .'" />';
		...
		case 'colorrgb':		
				...
				$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$ragnar_data[$value['id']].'" data-color-format="rgb"></div></div>';
				$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $ragnar_data[$value['id']] .'" />';
		...
		case 'sizeColor':
				if (!isset($ragnar_data[$value['id']])) {
					$ragnar_data[$value['id']] = array('size' => 12, 'color' => '#000000');
				}
				$sizeColor = $ragnar_data[$value['id']];
		...
		case 'font':
				if (!isset($ragnar_data[$value['id']])) {
					$ragnar_data[$value['id']] = array('face' => 'arial');
				}
				...
				foreach ($faces as $i=>$face) {
					$output .= '<option value="'. $i .'" ' . selected($ragnar_data[$value['id']], $i, false) . '>'. $face .'</option>';
				}			
		...
		case 'typography':
				if (!isset($ragnar_data[$value['id']])) {
					$ragnar_data[$value['id']] = array('size'=>12, 'face' => 'arial', 'color' => '#000000','style'=>'normal');
				}
				$typography_stored = $ragnar_data[$value['id']];
		...
		case 'border':
				...
				if(!isset($ragnar_data[$value['id'] . '_width'])) $ragnar_data[$value['id'] . '_width'] ='';
				if(!isset($ragnar_data[$value['id'] . '_style'])) $ragnar_data[$value['id'] . '_style'] ='';
				if(!isset($ragnar_data[$value['id'] . '_color'])) $ragnar_data[$value['id'] . '_color'] ='';
				
				$border_stored = array('width' => $ragnar_data[$value['id'] . '_width'] ,
										'style' => $ragnar_data[$value['id'] . '_style'],
										'color' => $ragnar_data[$value['id'] . '_color'],
										);
					
				/* Border Width */
				$border_stored = $ragnar_data[$value['id']];
		...
		case 'images':
				...
				$select_value = $ragnar_data[$value['id']];
...
/*-----------------------------------------------------------------------------------*/
/* Aquagraphite Uploader - ragnar_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
public static function ragnar_optionsframework_uploader_function($id,$std,$mod){
    $ragnar_data =get_option(RAGNAR_OPTIONS);
	
	$uploader = '';
	if(!empty($ragnar_data[$id])) {
    $upload = $ragnar_data[$id];
	}
	else {
	$upload = '';
	}
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'.esc_html__('Upload','ragnar').'</span>';
...
/*-----------------------------------------------------------------------------------*/
/* Aquagraphite Slider - ragnar_optionsframework_slider_function */
/*-----------------------------------------------------------------------------------*/
public static function ragnar_optionsframework_slider_function($id,$std,$oldorder,$order,$int, $social = false,  $menu = false, $sidebar = false){
    $ragnar_data = get_option(RAGNAR_OPTIONS);
	$slider = '';
 
	
	if(!empty($ragnar_data[$id])) {
     $slide = $ragnar_data[$id];
	}
	else {
	 $slide = '';
	}	
