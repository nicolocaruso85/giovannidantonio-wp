 ...
 */
add_action( 'widgets_init', 'ragnar_video_widget' );
/* Function that registers our widget. */
function ragnar_video_widget() {
	register_widget( 'ragnar_videos' );
}
class ragnar_videos extends WP_Widget {
	function ragnar_videos() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'videos', 'description' => esc_attr__('Displays the post image and title.','ragnar'));
		/* Create the widget. */
		parent::__construct( 'videos-widget',esc_attr__('Ragnar - Premium Video','ragnar'), $widget_ops, '' );
		...
		<div class="widgett <?php if(!empty($instance['self_video'])) echo 'self'; ?> id-<?php echo esc_attr($this->id); ?>">	
			<div class="video">
				...
				<?php if(!empty($instance['display_text'])) { ?>
					<div class="wttitle"><a target="_blank" href="<?php echo esc_attr($instance['link']); ?>" rel="bookmark" title="<?php esc_attr_e('Permanent Link to','ragnar')?> <?php echo esc_attr($instance['text']); ?>"><?php echo esc_attr($instance['text']);?></a></div>
				<?php } ?>
		</div>	
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Ragnar Video', 'video' => 'https://www.youtube.com/watch?v=35YZkJaDhh8', 'text' => 'Ragnar Video Text', 'text-top' => '200px','self_video'=>'','auto_play' => '','display_text' => '','link' =>'','self_video' => '','image' => '');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'video' )); ?>"><?php esc_attr_e('Video URL:','ragnar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Youtube or Vimeo or self hosted)','ragnar') ?></small>
		</p>
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'self_video' )); ?>"><?php esc_attr_e('Self hosted video (formats : "mp4", "m4v", "webm", "ogv", "wmv", "flv")?','ragnar') ?></label>
		</p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_name( 'image' )); ?>"><?php esc_attr_e('Image cover for self hosted video:','ragnar') ?></label>
            ...
            <input class="upload_image_button" type="button" value="<?php esc_attr_e('Upload Image','ragnar') ?>" />
        </p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_play' )); ?>"><?php esc_attr_e('Auto play (only for self hosted video)?','ragnar') ?></label>
		</p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'display_text' )); ?>"><?php esc_attr_e('Display Text with link?','ragnar') ?></label>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'link' )); ?>"><?php esc_attr_e('Link URL:','ragnar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Link for the text)','ragnar') ?></small>
		</p>			
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_attr_e('Text:','ragnar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Text over the video)','ragnar') ?></small>
		</p>		
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text-top' )); ?>"><?php esc_attr_e('Text bottom margin:','ragnar') ?></label>
			...
			<br /><small><?php esc_attr_e('(Bottom margin for text with px (200px))','ragnar') ?></small>
		</p>		
		<?php
	}
